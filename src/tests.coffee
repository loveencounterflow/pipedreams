...
@[ "(v4) $sort 6" ] = ( T, done ) ->
  ...
  TABLE               = require 'table'
  # CND.dir TABLE
  _new_table_stream   = TABLE.createStream
  table_settings      =
  ...
  sort = ( directions_and_keys..., matcher, handler ) =>
    new_table_stream = =>
      table_stream = _new_table_stream table_settings
      return $ ( data, send ) => table_stream.write data; send data
    urge directions_and_keys
    input = D.new_stream()
