...
(function() {
  var $as_event, $as_row, $cleanup, $dividers, $dividers_bottom, $dividers_top, $set_widths_etc, CND, D, _new_state, alert, as_row, as_text, badge, boxes, copy, debug, get_divider, help, keys_toplevel, ref, rpr, to_width, urge, values_alignment, values_overflow, warn, width_of;
  ...
  $set_widths_etc = function(S) {
    return D.$('first', function(event, send) {
  ...
  $dividers = function(S) {
    return D.new_stream({
      pipeline: [$dividers_top(S), $dividers_bottom(S)]
    });
  };
  ...
  $dividers_bottom = function(S) {
    return D.$('last', function(event, send) {
      send(event);
