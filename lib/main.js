...
(function() {
  ...
  * Wrap all stream/transform creation calls so that printing of shorthand descriptions of pipelines becomes
  ...
  this._rprx = function(start, symbol, name, extra, stop, stream) {
    var _brackets, _inspect, _symbols, arity, gl, gy, parts, show_names;
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    var R, encoding, expected, got, i, key, len, pipeline, ref, role, role_count, type, use_line_mode, x;
    ...
    if (role === 'read') {
      if (use_line_mode) {
        return this._rpr("🖹 △", "file-read", null, this.new_stream({
          pipeline: [
        ...
        return this._rpr("🖹 △", "file-read", null, this._new_stream$read_from_file(path, settings));
      }
    }
    if (encoding !== 'buffer') {
      if (settings['encoding'] == null) {
    ...
    if (role === 'append') {
      ...
      if (use_line_mode) {
        return this._rpr("🖹 ⏬", "file-append", null, this.new_stream({
          pipeline: [this.$as_line(), R]
        }));
      }
      return this._rpr("🖹 ⏬", "file-append", null, R);
    }

    /* role is write */
    R = this.$bridge(this._new_stream$write_to_file(path, settings));
    if (use_line_mode) {
      return this._rpr("🖹 ▼", "file-write", null, this.new_stream({
        pipeline: [this.$as_line(), R]
      }));
    }
    return this._rpr("🖹 ▼", "file-write", null, R);
    return null;
  };
  ...
  this.$split = function(settings) {
    ...
    if (encoding === 'buffer') {
      return this._rpr("✀", "split", null, R);
    }
    return this._rpr("✀", "split", null, this.new_stream({
  ...
  this.$bridge = function(stream) {
    ...
    return this._rpr('↷', 'bridge', rpr(stream), this.$((function(_this) {
      return function(data, send, end) {
        if (data != null) {
          stream.write(data);
          send(data);
