...
(function() {
  var CND, MSP, OLD, alert, badge, debug, echo, help, info, log, pluck, rnd_from_seed, rpr, urge, warn, whisper,
    slice = [].slice,
  ...
  OLD = function() {
    this.new_stream = function(settings) {
      var expected, got, kind, kinds, pipeline, text;
      if ((settings == null) || (kinds = Object.keys(settings)).length === 0) {
        return MSP.through.obj();
      }
      if ((text = pluck(settings, 'text')) != null) {
        return this._new_stream_from_text(text, settings);
      }
      if ((pipeline = pluck(settings, 'pipeline')) != null) {
        return this._new_stream_from_pipeline(pipeline, settings);
      }
      expected = ((function() {
        var i, len, ref, results;
        ref = this.new_stream.kinds;
        results = [];
        for (i = 0, len = ref.length; i < len; i++) {
          kind = ref[i];
          results.push(rpr(kind));
        }
        return results;
      }).call(this)).join(', ');
      got = ((function() {
    ...
    this.new_stream.kinds = ['file', 'text', 'pipeline'];
    return this._new_stream_from_text = function(text, settings) {
  ...
  this.new_stream = function() {
    var P, hints, kind, ref, seed, settings;
  ...
  this._new_stream_from_url = function(seed, hints, settings) {
