...
(function() {
  var CND, MSP, alert, badge, debug, echo, help, info, insp, log, pluck, rnd_from_seed, rpr, urge, warn, whisper,
    slice = [].slice,
  ...
  * Wrap all stream/transform creation calls so that printing of shorthand descriptions of pipelines becomes
  ...
  this._new_stream$line_splitter = function(matcher) {
    var R;
  ...
  this.new_stream = function() {
    ...
    this.new_stream._read_arguments_2(P);
  ...
  this.new_stream._read_arguments_2 = (function(_this) {
    return function(P) {
      var R, expected, flags_over, got, i, idx, kind, len, p, rprd, seed, x;
      R = {
        flags: [],
        kind: null,
        seed: null,
      ...
      for (idx = i = 0, len = P.length; i < len; idx = ++i) {
        ...
        if (CND.isa_text(p)) {
          ...
          if (R.kind == null) {
            ...
            R.settings = Object.assign.apply(Object, [{}].concat(slice.call(P.slice(idx))));
          }
        }
      }
      if (R.kind == null) {
  ...
  this._new_stream = function(seed, hints, settings) {
    ...
    return this._rpr("#", "plain", null, MSP.through.obj());
  };
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    var R, _encoding, encoding, expected, extra, got, i, key, len, pipeline, ref, role, role_count, type, use_line_mode, x;
  ...
  this._new_stream_from_pipeline = function(pipeline, hints, settings) {
     ...
     */
    var R, as_bridged, handler, inner_rpr, p, receiver, sender, transform, type;
    if (hints != null) {
    ...
    this._new_stream$pump(pipeline, handler);
    R = this.duplex(receiver, sender);
    inner_rpr = ((function() {
      var i, len, results;
      results = [];
      for (i = 0, len = pipeline.length; i < len; i++) {
  ...
  this.$pass_through = function() {
    return this._rpr("⦵", "pass-through", null, MSP.through.obj());
  };
  ...
  this._new_remit = function(tags, mode, method) {
    ...
    if (arity === 1) {
      ...
      return this._rpr("+", "$d", null, MSP.through.obj(main, flush));
    } else if (arity === 2) {
      if (send_null) {
        flush = function(callback) {
          var send;
    ...
    if (arity === 2) {
      return this._rpr("⧺", "$ds", null, MSP.through.obj(main, flush));
    }
    return this._rpr("⧻", "$dse", null, MSP.through.obj(main, flush));
  };
  ...
  this.$split_2 = function(settings) {
    var encoding, extra, includes, matcher, matcher_bfr, output, pipeline, ref, ref1, ref2, splitter, strip, type;
    matcher = (ref = settings != null ? settings['matcher'] : void 0) != null ? ref : '\n';
    if ((type = CND.type_of(matcher)) !== 'text') {
      throw new Error("expected a text, got a " + type);
    }
    strip = ((ref1 = settings != null ? settings['strip'] : void 0) != null ? ref1 : true) && matcher.length > 0;
    encoding = (ref2 = settings != null ? settings['encoding'] : void 0) != null ? ref2 : 'utf-8';
    if (strip) {
      matcher_bfr = Buffer.isBuffer(matcher) ? matcher : new Buffer(matcher, 'utf-8');
    }
    if (Buffer.prototype.includes != null) {
      includes = function(buffer, probe) {
        return buffer.includes(probe, buffer.length - probe.length);
      };
    } else {
      includes = function(buffer, probe) {
        return ((buffer.slice(buffer.length - probe.length)).compare(probe)) === 0;
      };
    }
    splitter = this._new_stream$line_splitter(matcher);
    output = this.new_stream();
    splitter.on('data', (function(_this) {
      return function(data) {
        return _this.send(output, data);
      };
    })(this));
    splitter.on('fragment', (function(_this) {
      return function(data) {
        return _this.send(output, data);
      };
    })(this));
    splitter.on('end', (function(_this) {
      return function() {
    ...
    if (strip) {
      pipeline.push(this.$((function(_this) {
        return function(data, send) {
          if (!includes(data, matcher_bfr)) {
            return send(data);
          }
          return send(data.slice(0, data.length - matcher_bfr.length));
        };
      })(this)));
    }
    if (encoding !== 'buffer') {
      pipeline.push(this.$decode(encoding));
    }
    extra = rpr(matcher);
    if (encoding !== 'buffer') {
      extra += " " + encoding;
  ...
  this.$benchmark = function(title) {
    ...
    R.on('data', (function(_this) {
      return function() {
        ...
        me.n += 1;
        if (me.n % 10000 === 0) {
          return echo(title, me.n);
        }
      };
    })(this));
    R.on('finish', (function(_this) {
      return function() {
  ...
  this.$bridge = function(stream) {
    ...
    meaning.
     */
    var R, arity, bridge, extra, handler, output, throughput;
    ...
    output = this.new_stream();
    throughput = this.$((function(_this) {
      return function(data, send, end) {
        if (data != null) {
          send(data);
          output.write(data);
    ...
    bridge = this._new_stream$pump([throughput, stream], handler);
    R = this.duplex(bridge, output);
    extra = (rpr(bridge)) + ' ↝ ' + (rpr(output));
    return this._rpr("↷", "bridge", extra, R);
  };

  this.duplex = function(receiver, sender) {
