...
(function() {
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    var expected, fs_settings, got, pipeline, role, role_count, type, use_line_mode, x;
    ...
    fs_settings = {};
    if (hints != null) {
      if ((indexOf.call(hints, 'utf8') >= 0) || (indexOf.call(hints, 'utf-8') >= 0)) {
    ...
    if (role === 'read') {
      ...
      pipeline.push(this.$bridge((require('fs')).createWriteStream(path, fs_settings)));
    }
    return this.new_stream({
      pipeline: pipeline
    });
  ...
  this._new_stream_from_url = function(seed, hints, settings) {
    var HTTP, R, http_settings, request;
    HTTP = require('http');
    http_settings = {
      host: 'example.com',
      path: '/'
