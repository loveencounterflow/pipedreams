...
(function() {
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    ...
    if (role === 'read') {
      if (use_line_mode) {
        pipeline.push(this._new_fs_read_stream(path, settings));
      ...
      pipeline.push(this.$bridge(this._new_fs_write_stream(path, settings)));
    }
    return this.new_stream({
      pipeline: pipeline
    });
  ...
  this._new_fs_read_stream = function() {
    var P, ref;
    P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    return this._wrap_stream('FR', (ref = require('fs')).createReadStream.apply(ref, P));
  };

  this._new_fs_write_stream = function() {
    var P, ref;
    P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    return this._wrap_stream('FW', (ref = require('fs')).createWriteStream.apply(ref, P));
  };
