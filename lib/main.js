...
(function() {
  var CND, MSP, alert, badge, debug, echo, help, info, log, pluck, rnd_from_seed, rpr, stringfoo, urge, warn, whisper,
    slice = [].slice;
  ...
  this.new_stream = function(settings) {
    var expected, file, got, key, keys, pipeline, text;
    if ((settings == null) || (keys = Object.keys).length === 0) {
      return MSP.through.obj();
    }
    if ((file = pluck(settings, 'file')) != null) {
      return this.new_stream_from_file(file, settings);
    }
    if ((text = pluck(settings, 'text')) != null) {
      return this.new_stream_from_text(text, settings);
    }
    if ((pipeline = pluck(settings, 'pipeline')) != null) {
      return this.new_stream_from_pipeline(pipeline, settings);
    }
    expected = ((function() {
      var i, len, ref, results;
      ref = this.new_stream.keys;
      results = [];
  ...
  this.new_stream.keys = ['file', 'text', 'pipeline'];
  ...
  this.new_stream_from_text = function(text, settings) {
  ...
  this.new_stream_from_pipeline = function(pipeline, settings) {
     ...
     */
    var i, idx, len, sink, source, transform, type;
    ...
    sink = source;
    for (idx = i = 0, len = pipeline.length; i < len; idx = ++i) {
      transform = pipeline[idx];
      sink = sink.pipe(transform);
    }
    return MSP.duplex(source, sink, {
      objectMode: true
    });
  };

  this.new_file_readstream = function() {
    throw new Error("new_file_readstream not implemented");
  };

  this.new_file_readlinestream = function() {
    throw new Error("new_file_readlinestream not implemented");
  };
  ...
  this._$async_single = function(method) {
    var arity;
  ...
  this.$async = function(method) {
    var $call, $spread, $wait_for_stream_end, Z, _send_end, _stream_end, arity, has_end_argument, input, output, ref;
    if (!((2 <= (ref = (arity = method.length)) && ref <= 3))) {
      throw new Error("expected a method with an arity of 2 or 3, got one with an arity of " + arity);
    }
    Z = [];
    has_end_argument = arity === 3;
    input = this.new_stream();
    output = this.new_stream();
    _send_end = null;
    _stream_end = null;
    $wait_for_stream_end = (function(_this) {
      return function() {
    ...
    $call = (function(_this) {
      return function() {
        return _this._$async_single(function(event, done) {
          var _send;
          _send = function(data) {
            Z.push(data);
            return null;
    ...
    $spread = (function(_this) {
      return function() {
        return _this.$(function(collection, send, end) {
          var event, i, len;
    ...
    input.pipe($wait_for_stream_end()).pipe($call()).pipe($spread()).pipe(output);
    return this.new_stream({
      pipeline: [input, output]
    });
  };

  this.$async_OLD = function(method) {
    var arity;
    if ((arity = method.length) !== 2) {
      throw new Error("expected a method with an arity of 2, got one with an arity of " + arity);
    }
  ...
  this.$join = function(joiner) {
     ...
     */
    return this.combine([
      this.$collect(), this.$((function(_this) {
  ...
  this.$on_end = function(method) {
    ...
    if (!((0 <= (ref = (arity = method.length)) && ref <= 1))) {
      throw new Error("expected 0 or 1 argument, got " + arity);
    }
