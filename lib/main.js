...
(function() {
  ...
  * Wrap all stream/transform creation calls so that printing of shorthand descriptions of pipelines becomes
  ...
  this._new_stream$throttle_bytes = function(bytes_per_second) {
    ...
    return this._rpr("⏳", "throttle", bytes_per_second + " B/s", R);
  };
  ...
  this._new_stream_from_pipeline = function(pipeline, hints, settings) {
    ...
    return this._rprx("[", null, "pipeline", inner, "]", (ref = MSP.pipeline).obj.apply(ref, pipeline));
  };
  ...
  this.$sort = function(sorter, settings) {
    ...
    return this._rpr("⮃", "sort", null, this.$((function(_this) {
  ...
  this.$intersperse = function() {
    var R, arity, cache, call_first_joiner, call_joiner, call_last_joiner, call_mid_joiner, extra, first_joiner, joiners, last_joiner, mid_joiner, ref;
    joiners = 1 <= arguments.length ? slice.call(arguments, 0) : [];

    /* Similar to `$join`,
     */
    if (!((1 <= (ref = (arity = arguments.length)) && ref <= 3))) {
      throw new Error("expected 1 to 3 joiners, got " + arity);
    }
    cache = null;
    call_joiner = false;
    first_joiner = null;
    ...
    (function(_this) {
      return (function() {
        var i, idx, joiner, len, results, type;
        results = [];
        for (idx = i = 0, len = joiners.length; i < len; idx = ++i) {
          ...
          switch (type = CND.type_of(joiner)) {
            ...
            case 'function':
              ...
              break;
            default:
              throw new Error("expected a text or a function, got a " + type);
          }
          switch (idx) {
            case 0:
              mid_joiner = joiner;
              results.push(call_mid_joiner = type === 'function');
              break;
            case 1:
            ...
            case 2:
              last_joiner = joiner;
              results.push(call_last_joiner = type === 'function');
              break;
            default:
              results.push(void 0);
          }
    ...
    R = this.$((function(_this) {
      return function(data, send, end) {
        if (data != null) {
          if (cache != null) {
            send(cache);
            send(call_mid_joiner ? mid_joiner(data, cache) : mid_joiner);
          } else {
            if (first_joiner != null) {
              send(call_first_joiner ? first_joiner(null, data) : first_joiner);
            }
          }
          cache = data;
        }
        if (end != null) {
          if (cache != null) {
            send(cache);
            if (last_joiner != null) {
              send(call_last_joiner ? last_joiner(null, data) : last_joiner);
            }
          }
          cache = null;
          return end();
  ...
  this.$as_json_list = function() {
    ...
    translate_null = this.$((function(_this) {
      return function(data, send) {
        return send(data === NULL ? 'null' : data);
      };
    })(this));
    R = this.new_stream({
      pipeline: [this.$stringify(), intersperse, translate_null, this.$show(), this.$join('')]
    });
