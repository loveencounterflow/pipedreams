...
(function() {
  ...
  this._new_stream = function(seed, hints, settings) {
    ...
    return this._wrap_stream('P', MSP.through.obj());
  };

  this._new_stream$fs_read_stream = function() {
    var P, ref;
    P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    return this._wrap_stream('FsR', (ref = require('fs')).createReadStream.apply(ref, P));
  };

  this._new_stream$fs_write_stream = function() {
    var P, ref;
    P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    return this._wrap_stream('FsW', (ref = require('fs')).createWriteStream.apply(ref, P));
  };

  this._new_stream$split_buffer = function(matcher) {
    return this._wrap_stream("//" + (rpr(matcher)) + "//", (require('binary-split'))(matcher));
  };

  this._wrap_stream = function(sigil, stream) {
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    ...
    if (role === 'read') {
      if (use_line_mode) {
        pipeline.push(this._new_stream$fs_read_stream(path, settings));
      ...
      pipeline.push(this.$bridge(this._new_stream$fs_write_stream(path, settings)));
    }
    return this.new_stream({
      pipeline: pipeline
    });
  ...
  this._new_stream_from_pipeline = function(pipeline, hints, settings) {
    ...
    return this._wrap_stream(inspect, (ref = MSP.pipeline).obj.apply(ref, pipeline));
  };
  ...
  this.$pass_through = function() {
    return this._wrap_stream('PT', MSP.through.obj());
  };
  ...
  this._new_remit = function(mode, method) {
    ...
    if (arity === 1) {
      ...
      return this._wrap_stream('t', MSP.through.obj(main, flush));
    }
    if (arity === 3) {
      flush = function(callback) {
  ...
  this.$throttle_bytes = function(bytes_per_second) {
    return this._wrap_stream("throttle " + bytes_per_second + " bps", new (require('throttle'))(bytes_per_second));
  };
