...
(function() {
  var CND, alert, badge, debug, echo, help, info, insp, log, rpr, urge, warn, whisper, x_count,
    slice = [].slice,
  ...
  * Wrap all stream/transform creation calls so that printing of shorthand descriptions of pipelines becomes
  ...
  this._new_stream_from_path = function(path, hints, settings) {
    var R, _encoding, encoding, extra, i, key, len, pipeline, ref, role, role_count, show_progress_bar, type, use_line_mode;
    ...
    if (hints != null) {
      ...
      if (indexOf.call(hints, 'progress') >= 0) {
        if (role !== 'read') {
          throw new Error("hint 'progress' can only be used with readstreams");
        }
        show_progress_bar = this._new_stream_from_path.$_show_progress_bar(path);
      }
    }
    ...
    if (role === 'read') {
      if (use_line_mode) {
        ...
        if (show_progress_bar != null) {
          pipeline.push(show_progress_bar);
        }
  ...
  this._new_stream_from_path.$_show_progress_bar = (function(_this) {
    return function(path) {
      ...
      return _this.$(function(data) {
        return bar.tick(data.length);
      });
    };
  })(this);

  this._new_stream_from_path._hints = ['ascii', 'utf8', 'utf-8', 'ucs2', 'base64', 'binary', 'hex', 'buffer', 'read', 'write', 'append', 'progress', 'lines'];
  ...
  this.$read_file = function(settings) {

    /* TAINT introduce maximum size setting */
    var R, bare, encoding, ref, ref1;
    encoding = (ref = settings != null ? settings['encoding'] : void 0) != null ? ref : null;
    bare = (ref1 = settings != null ? settings['bare'] : void 0) != null ? ref1 : false;
    R = this.$async((function(_this) {
      return function(path, send, end) {
        var input;
        if (path != null) {
          input = _this.new_stream({
            path: path
          });
          input.pipe(_this.$collect()).pipe(_this.$(function(chunks) {

            /* TAINT make PipeDreams method */
            var Z, length;
            length = chunks.reduce((function(x, chunk) {
              return x + chunk.length;
            }), 0);
            Z = (require('buffer')).Buffer.concat(chunks, length);
            if (encoding != null) {
              Z = Z.toString(encoding);
            }
            send.done(bare ? Z : [path, Z]);
            if (end != null) {
              return end();
            }
          }));
  ...
  this.$as_json_list = function() {
    ...
    translate_null = this.$((function(_this) {
      return function(data, send) {
        return send(data === _this.σ_null ? 'null' : data);
      };
    })(this));
    R = this.new_stream({
  ...
  this.$tap = function(stream, settings) {
    ...
    switch (copy) {
      ...
      case true:
        ...
        break;
      default:
        if ((type = CND.type_of(copy)) !== 'function') {
