// Generated by CoffeeScript 2.3.1
(function() {
  'use strict';
  var $, $async, CND, L, PD, PS, assign, badge, copy, debug, echo, help, info, jr, override_sym, rpr, select, stamp, urge, warn, whisper;

  //###########################################################################################################
  CND = require('cnd');

  rpr = CND.rpr;

  badge = 'PIPEDREAMS/OVERRIDES';

  debug = CND.get_logger('debug', badge);

  warn = CND.get_logger('warn', badge);

  info = CND.get_logger('info', badge);

  urge = CND.get_logger('urge', badge);

  help = CND.get_logger('help', badge);

  whisper = CND.get_logger('whisper', badge);

  echo = CND.echo.bind(CND);

  ({assign, jr} = CND);

  //...........................................................................................................
  PS = require('pipestreams');

  PD = require('..');

  ({$, $async, select, stamp} = PD);

  //...........................................................................................................
  ({jr, copy, assign} = CND);

  //...........................................................................................................
  override_sym = Symbol.for('override');

  //-----------------------------------------------------------------------------------------------------------
  this.$collect = function(settings) {
    var collect, defaults, expedite, get_value, last_sym, ref, send, type, use_custom_collector;
    defaults = {
      select: null,
      key: 'collection',
      callback: null,
      value: null,
      collector: null
    };
    settings = assign({}, defaults, settings);
    use_custom_collector = settings.collector != null;
    send = null;
    last_sym = Symbol('last');
    //.........................................................................................................
    switch ((type = CND.type_of((ref = settings.value) != null ? ref : null))) {
      case 'null':
        get_value = function(d) {
          return d;
        };
        break;
      case 'boolean':
        if (settings.value) {
          get_value = function(d) {
            return d.value;
          };
        } else {
          get_value = function(d) {
            return d;
          };
        }
        break;
      case 'text':
        get_value = function(d) {
          return d[settings.value];
        };
        break;
      case 'function':
        get_value = settings.value;
        break;
      default:
        throw new Error(`µ20922 expected a boolean, a text or a function, got a ${type}`);
    }
    //.........................................................................................................
    expedite = function() {
      // debug '37763-1->', collector, settings.callback, settings.key
      if (settings.collector == null) {
        return;
      }
      if (settings.callback != null) {
        settings.callback(settings.collector);
      } else {
        send(PD.new_single_event(settings.key, settings.collector));
      }
      return settings.collector = null;
    };
    //.........................................................................................................
    collect = function(d) {
      var ref1;
      if (settings.collector == null) {
        settings.collector = [];
      }
      settings.collector.push((ref1 = get_value(d)) != null ? ref1 : null);
      return null;
    };
    //.........................................................................................................
    return $({
      last: last_sym
    }, function(d, _send) {
      send = _send;
      //.......................................................................................................
      if (d === last_sym) {
        expedite();
      //.......................................................................................................
      } else if (select(d, '~collect')) {
        if (use_custom_collector) {
          throw new Error("µ110299 unable to use `~collect` symbol with custom collector");
        }
        expedite();
      } else {
        if ((settings.select == null) && (!PD.is_system(d))) {
          //.......................................................................................................
          return collect(d);
        }
        if ((settings.select != null) && (select(d, settings.select))) {
          return collect(d);
        }
        /* At this point we know the current datom will not be collected, so we first send whatever we have
        collected, if anything, in order not to mess up the relative ordering of events. */
        expedite();
        send(d);
      }
      //.......................................................................................................
      return null;
    });
  };

  //-----------------------------------------------------------------------------------------------------------
  this.$drain = function(...P) {
    var pipeline;
    pipeline = [];
    pipeline.push($(function(d, send) {
      if (select(d, '~end')) {
        return send(PD.symbols.end);
      } else {
        return send(d);
      }
    }));
    pipeline.push(PS.$drain(...P));
    return PD.pull(...pipeline);
  };

  //###########################################################################################################
  L = this;

  (function() {
    var key, results, value;
/* Mark all methods defined here as overrides: */
    results = [];
    for (key in L) {
      value = L[key];
      results.push(value[override_sym] = true);
    }
    return results;
  })();

}).call(this);

//# sourceMappingURL=overrides.js.map
