...
(function() {
  'use strict';
  var $, $async, CND, PD, XE, alert, badge, computation_listener, debug, defer, help, info, jr, rpr, urge, warn, whisper;
  ...
  XE.listen_to_all(this, function(key, event) {
    var k;
    whisper('µ28823-1', key, event, (function() {
      var results;
      results = [];
      for (k in this) {
        results.push(k);
  ...
  XE.listen_to_all((key, event) => {
    var k;
    whisper('µ28823-2', key, event, (function() {
      var results;
      results = [];
      for (k in this) {
        results.push(k);
  ...
  (async() => {    // #-----------------------------------------------------------------------------------------------------------
    ...
    datom: */
    info('µ28823-4', (await XE.emit(PD.new_event('^plus-sync', {
      a: 42,
      b: 108
    }))));
    // -> [ 'listener #4', { key: '~xemitter-preferred', value: 150 }, 'listener #1', 'listener #2' ]
    info('µ28823-5', (await XE.emit(PD.new_event('^plus-async', {
      a: 42,
      b: 108
    }))));
    // -> [ 'listener #4', { key: '~xemitter-preferred', value: 150 }, 'listener #1', 'listener #2' ]
    ...
    info('µ28823-6', (await XE.delegate(PD.new_event('^plus-async', {
      a: 42,
      b: 108
    }))));
    try {
      ...
      return info('µ28823-7', (await XE.delegate(PD.new_event('^unknown-event', 'some value'))));
    } catch (error1) {
      error = error1;
      return warn('µ28823-8', error.message);
    }
  })();
