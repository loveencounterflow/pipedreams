...
(function() {
  ...
  this["(v4) $switch (study)"] = function(T, done) {
    var even_multiplier_and_adder, i, input, inputs, j, k, len, len1, make_it_so, n, odd_divider, output;
    ...
    make_it_so = function(ONE_OF_THE_STREAMS) {
      var input;
      input = D.new_stream();
      input.pipe((function() {
        var _end, _send, foo;
        ...
        _end = null;
        foo = D.new_stream();
        foo.pipe(ONE_OF_THE_STREAMS).pipe($(function(data, send, end) {
          if (data != null) {
            _send(data);
          }
          if (end != null) {
    ...
    inputs = [];
    inputs.push(make_it_so(even_multiplier_and_adder));
    inputs.push(make_it_so(odd_divider));
    for (n = i = 0; i <= 5; n = ++i) {
