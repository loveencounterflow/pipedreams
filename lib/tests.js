...
(function() {
  ...
  if (module.parent == null) {
    ...
    /* ----------------===#O#===-------------------- */
    include = ["(v4) grep: new stream from pattern and path (basic pattern)", "(v4) grep: new stream from pattern and path (with ignore-case)", "(v4) grep: new stream from pattern and path (no results without ignore-case)", "(v4) grep: new stream from pattern and path (error with non-existent file)"];
    this._prune();
    this._main();
  }
