...
(function() {
  ...
  this["(v4) file stream events (1)"] = function(T, done) {
    var MSP, matcher, path_1, probes, step, write_sample;
    step = (require('coffeenode-suspend')).step;
    path_1 = resolve_temp_path('_new_stream_from_path-4.txt');
    probes = ['helo', 'world', '𪉟⿱鹵皿'];
    matcher = ['helo', 'world', '𪉟⿱鹵皿'];
    ...
    write_sample = (function(_this) {
      return function(handler) {
        var i, input, len, output, pipeline, probe, thruput;
        input = MSP.through.obj();
        thruput = MSP.through.obj();
        output = D.new_stream('append', {
          file: path_1
        });
        pipeline = input.pipe(D.$as_line()).pipe(D.$show()).pipe(output).pipe(thruput).pipe($(function(data) {
          if (data != null) {
            return send(data);
          }
          return debug(CND.green('transform end'));
        }));
  ...
  if (module.parent == null) {
    include = ["(v4) file stream events (1)"];
    this._prune();
    this._main();
  }
