...
(function() {
  ...
  this["(v4) stream / transform construction with through2"] = function(T, T_done) {
    ...
    transform_main = function(line, encoding, handler) {
      if (encoding !== 'utf-8') {
        throw new Error("unknwon encoding " + (rpr(encoding)));
      }
    ...
    return input.pipe(D.$split()).pipe(through2.obj(t2_settings, transform_main, transform_flush)).pipe(D.$show()).pipe(D.$on_end((function(_this) {
      return function() {
        return T_done();
      };
    })(this)));
