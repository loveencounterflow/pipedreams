...
(function() {
  ...
  this["(v4) $select (1)"] = function(T, done) {
    var draw_a_separator, i, len, matchers, my_input, probe, probes, say_it_in_english, say_it_in_french, say_it_in_german, selector, tracks;
    ...
    my_input.pipe(D.$select(selector, tracks)).pipe(D.$collect()).pipe($((function(_this) {
  ...
  this["(v4) $select (2)"] = function(T, done) {
    var bridgehead, draw_a_separator, draw_line_track, drop, i, language_track, len, matchers, my_input, probe, probes, say_it_in_english, say_it_in_french, say_it_in_german, select_draw_line_events, select_drop_and_pass_events, select_language_events;
    ...
    bridgehead = D.$pass();
    drop = D.$drop();
    select_drop_and_pass_events = function(event) {
      if (event === 'drop this one') {
        return {
          key: drop
        };
    ...
    select_draw_line_events = function(event) {
      if (event !== '---') {
        return {
          key: Symbol["for"]('pass')
        };
    ...
    select_language_events = function(event) {
      var language, languages, number;
    ...
    my_input = D.new_stream();
    my_input.pipe(D.$select(select_drop_and_pass_events)).pipe(D.$select(select_draw_line_events, draw_line_track)).pipe(D.$select(select_language_events, language_track)).pipe(bridgehead).pipe(D.$collect()).pipe($((function(_this) {
      return function(results) {
        var i, idx, len, matcher, results1;
        T.eq(results.length, matchers.length);
        results1 = [];
