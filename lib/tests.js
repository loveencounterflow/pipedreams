...
(function() {
  ...
  this["(v4) $as_tsv"] = function(T, done) {
    var f, i, len, path, probe, probes, step, to_width, write;
    step = require('coffeenode-suspend').step;
    to_width = require('to-width').to_width;
    path = '/tmp/$as_tsv.tsv';
    f = function() {
      this.$as_tsv = function() {
        var i, names, ref, settings, stringify;
        names = 2 <= arguments.length ? slice.call(arguments, 0, i = arguments.length - 1) : (i = 0, []), settings = arguments[i++];
        if (CND.isa_text(settings)) {
          names.push(settings);
          settings = null;
        }
        stringify = (ref = settings != null ? settings['stringify'] : void 0) != null ? ref : null;
        if (stringify == null) {
      ...
      return this.$as_tsv = this._rpr("as-tsv", "as-tsv", null, this.$as_tsv);
    };
    f.apply(D);
    write = (function(_this) {
      return function(handler) {
        ...
        input.pipe(D.$show('1')).pipe(D.$as_list('date', 'size', 'name')).pipe(D.$show('2')).pipe(D.$as_tsv('date', 'size', 'name')).pipe(D.$show('3')).pipe(D.$finish(output, handler));
        for (i = 0, len = probes.length; i < len; i++) {
          probe = probes[i];
          D.send(input, probe);
        }
