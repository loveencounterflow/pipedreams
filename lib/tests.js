...
(function() {
  ...
  this["(v4) $select (2)"] = function(T, done) {
    ...
    matchers = ["un", "deux", "troix", "beaucoup", "pass this one", "—————", "one", "un", "—————", "one", "un", "eins", "—————", "two", "—————", "drei", "viele", "guess we're done here"];
    my_input = D.new_stream();
    my_input.pipe(D.$select(select_drop_and_pass_events)).pipe(D.$select(select_language_events, language_track)).pipe(bridgehead).pipe($((function(_this) {
      return function(data) {
        return urge(JSON.stringify(data));
      };
    })(this))).pipe(D.$collect()).pipe($('finish', done));
    for (i = 0, len = probes.length; i < len; i++) {
      probe = probes[i];
      D.send(my_input, probe);
    }
