...
(function() {
  ...
  this["(v4) README demo (1)"] = function(T, done) {
    ...
    })(this))).pipe($summarize("position #1:")).pipe($as_text_line()).pipe($verify()).pipe($summarize("position #2:"));
    D.on_finish(input, done);
    ref = [4, 7, 9, 3, 5, 6];
  ...
  this["(v4) $sort 6"] = function(T, done) {
    var i, len, probe, probes, sort, step;
    step = (require('coffeenode-suspend')).step;
    sort = (function(_this) {
      return function() {
        var direction, directions_and_keys, handler, i, input, j, key, len, matcher, pipeline, probe;
        ...
        input.pipe(D.new_stream({
          ...
          if (data != null) {
            return info(JSON.stringify(data));
          }
        })).pipe($(function(data, send) {
          return send([data['date'], data['size'], data['name']]);
        }));
        D.on_finish(input, handler);
  ...
  this["(v4) $as_json_list (2b)"] = function(T, done) {
    ...
    f = function(path, handler) {
      ...
      D.on_finish(output, handler);
      source.pipe($serialize()).pipe($insert_delimiters()).pipe($start_list()).pipe($stop_list()).pipe($as_text()).pipe(output);
