...
(function() {
  ...
  this["(v4) $tap, $as_json_line"] = function(T, done) {
    var $cast_1, $cast_2, by_stream, input, path;
    path = resolve_path(__dirname, '../test-data/files.tsv');
    input = D.new_stream({
      path: path
    });
    by_stream = D.new_stream({
      pipeline: [
    ...
    return input.pipe(D.$split_tsv({
      ...
      seed: 998791
    })).pipe($cast_1()).pipe(D.$tap(by_stream)).pipe($cast_2()).pipe($((function(_this) {
      return function(data) {
  ...
  if (module.parent == null) {
    ...
    /* ----------------===#O#===-------------------- */
    include = ["(v4) tap", "(v4) $tap, $as_json_line"];
    this._prune();
    this._main();
  }
