...
(function() {
  ...
  if (module.parent == null) {
    include = ["(v4) new_stream_from_pipeline (1a)", "(v4) new_stream_from_pipeline (3)", "(v4) new_stream_from_pipeline (4)", "(v4) D.new_stream", "(v4) D.new_stream_from_pipeline", "(v4) stream / transform construction with through2 (1)", "(v4) new_stream_from_text", "(v4) new_stream_from_text doesn't work synchronously", "(v4) new_stream_from_text (2)", "(v4) README demo (1)", "(v4) observer transform called with data `null` on stream end", "(v4) $async with method arity 2", "(v4) $async with method arity 3"];
    this._prune();
    this._main();
  }
