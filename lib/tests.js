...
(function() {
  ...
  this["(v4) $on_first, $on_last not called in empty stream (2)"] = function(T, done) {
    var count, input;
    count = 0;
    input = D.new_stream();
    input.pipe(D.$on_first(function(event, send) {
      count += +1;
      warn("D.$on_first called in empty stream", event);
      return send(event);
    })).pipe(D.$on_last(function(event, send) {
      count += +1;
      warn("D.$on_last called in empty stream", event);
      return send(event);
    })).pipe(D.$on_finish((function(_this) {
    ...
    $colorize = (function(_this) {
      return function() {
        return $(function(row, send) {
          ...
          return row['name'] = CND.lime(row['name']);
        });
      };
    })(this);
    show = (function(_this) {
  ...
  if (module.parent == null) {
    include = ["(empty-string) $tabulate", "(v4) $on_first, $on_last not called in empty stream (1)", "(v4) $on_first, $on_last not called in empty stream (2)"];
    this._prune();
    this._main();
  }
