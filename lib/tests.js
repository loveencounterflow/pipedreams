...
(function() {
  var $, $async, CND, D, alert, badge, debug, echo, get_index, help, include, info, isa_stream, log, rpr, stream, test, urge, warn, whisper,
  ...
  this["(v4) stream / transform construction with through2 (2)"] = function(T, T_done) {
    var $collect, $retrieve_data_from_db, MSP, S, db, delay, glyph, i, len, read_one_phrase, ref, t2_settings;
    MSP = require('mississippi');
    t2_settings = {};
    S = {};
  ...
  if (module.parent == null) {
    include = ["(v4) new_stream_from_pipeline (1a)", "(v4) new_stream_from_pipeline (3)", "(v4) new_stream_from_pipeline (4)", "(v4) D.new_stream", "(v4) D.new_stream_from_pipeline", "(v4) stream / transform construction with through2 (1)", "(v4) stream / transform construction with through2 (2)", "(v4) new_stream_from_text", "(v4) new_stream_from_text doesn't work synchronously", "(v4) new_stream_from_text (2)", "(v4) README demo (1)", "(v4) observer transform called with data `null` on stream end"];
    stream = D.new_stream();
    info(stream.end);
    info(stream.close);
    info(stream.finish);
