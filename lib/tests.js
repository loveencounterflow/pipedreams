...
(function() {
  ...
  this["(v4) $select (2)"] = function(T, done) {
    var draw_a_separator, draw_line_track, i, language_track, len, matchers, my_input, probe, probes, say_it_in_english, say_it_in_french, say_it_in_german, select_draw_line_events, select_language_events;
    say_it_in_english = $(function(n, send, end) {
      if (n != null) {
        switch (n) {
          case 1:
    ...
    matchers = ["un", "deux", "troix", "beaucoup", "—————", "one", "un", "—————", "one", "un", "eins", "—————", "two", "—————", "drei", "viele", "guess we're done here"];
    my_input = D.new_stream();
    my_input.pipe(D.$select(select_draw_line_events, draw_line_track)).pipe(D.$select(select_language_events, language_track)).pipe($((function(_this) {
      return function(data) {
