...
(function() {
  ...
  this["(empty-string) can send empty strings (validate mississippi pipe works as such)"] = function(T, done) {
    var MSP, handler, input, matchers, sub_input, sub_output, sub_thruput, through;
    MSP = require('mississippi');
    matchers = [];
    sub_input = D.new_stream();
    sub_thruput = D.$show();
    ...
    handler = (function(_this) {
      return function(error) {
        ...
        return T.succeed("mississippi pipe ok.");
      };
    })(this);
  ...
  if (module.parent == null) {
    include = ["(empty-string) can send empty strings (w/out pipeline)", "(empty-string) can send empty strings (w/ pipeline)", "(empty-string) can send empty strings (w/ mississippi pipeline)", "(empty-string) can send empty strings (validate mississippi duplex works as such)", "(empty-string) can send empty strings (validate mississippi duplex works with empty strings)", "(empty-string) can send empty strings (validate mississippi pipe works as such)"];
    this._prune();
    this._main();
  }
