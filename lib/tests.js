...
(function() {
  var $, $async, $split_tsv, CND, D, TMP, _temp_folder, alert, badge, debug, delay, echo, help, include, info, isa_stream, log, resolve_path, resolve_temp_path, rpr, temp_home, test, urge, warn, whisper,
    slice = [].slice,
  ...
  this["(v4) new new_stream signature (1)"] = function(T, done) {
    ...
    matchers = [
      ...
      ], ["text", "make it so", null, null, null], ["text", "make it so", ["oops"], null, null], ["*plain", null, ["text", "make it so"], null, null], ["*plain", null, ["binary", "append", "~/some-file.txt"], null, null], ["file", "~/some-file.txt", ["omg", "append"], null, null], [null, null, null, null, "expected a 'kind' out of '*plain', 'file', 'path', 'pipeline', 'text', 'url', got 'route'"]
    ];
  ...
  this["(v4) _new_stream_from_path (interim) (2)"] = function(T, done) {
    var MSP, matcher, path_1, probes, read_sample, step, write_sample;
    step = (require('coffeenode-suspend')).step;
    ...
    write_sample = (function(_this) {
      return function(handler) {
        var A, W, Z, i, input, len, output, probe;
        input = D.new_stream();
        W = (require('fs')).createWriteStream(path_1);
        Z = D.new_stream();
        A = $(function(data, send, end) {
          if (data != null) {
            W.write(data);
            Z.write(data);
          }
        ...
        output = MSP.duplex(A, Z, {
          objectMode: true
        });
        input.pipe($(function(line, send) {
          return send(line + '\n');
        })).pipe(output).pipe(D.$show()).pipe(D.$on_end(function() {
          return handler();
        }));
        for (i = 0, len = probes.length; i < len; i++) {
          probe = probes[i];
  ...
  this["(v4) _new_stream_from_path (2)"] = function(T, done) {
    ...
    write_sample = (function(_this) {
      return function(handler) {
        ...
        input.pipe(D.$show()).pipe(D.$as_line()).pipe(D.$bridge(output)).pipe(D.$on_end(function() {
          return handler();
        }));
        for (i = 0, len = probes.length; i < len; i++) {
          probe = probes[i];
  ...
  this["(v4) _new_stream_from_path (3)"] = function(T, done) {
    ...
    write_sample = (function(_this) {
      return function(handler) {
        ...
        input.pipe(D.$show()).pipe(output).pipe(D.$on_end(function() {
          return handler();
        }));
        for (i = 0, len = probes.length; i < len; i++) {
          probe = probes[i];
