...
(function() {
  ...
  this["(v4) stream / transform construction with through2 (1)"] = function(T, T_done) {
    var FS, MSP, PATH, input, t2_settings, transform_flush, transform_main;
    FS = require('fs');
    PATH = require('path');
    MSP = require('mississippi');
    t2_settings = {};
  ...
  this["(v4) stream / transform construction with through2 (2)"] = function(T, T_done) {
    var $collect, $finalize, $retrieve_data_from_db, MSP, S, db, glyph, i, len, read_phrases, ref, t2_settings;
    MSP = require('mississippi');
    t2_settings = {};
    S = {};
  ...
  this["(v4) $sort 6"] = function(T, done) {
    ...
    $tabulate = (function(_this) {
      return function() {
        return D.new_stream({
          pipeline: [
            $tabulate_row(), D.$on_start(function(send) {
    ...
    sort = (function(_this) {
      return function() {
        ...
        })).pipe(D.$benchmark(title)).pipe(D.$on_finish(handler));
      };
    })(this);
    return step((function(_this) {
      return function*(resume) {
  ...
  this["(v4) $as_tsv"] = function(T, done) {
    ...
    write = (function(_this) {
      return function(handler) {
        ...
        input = D.new_stream();
        input.pipe(D.$as_list('date', 'size', 'name')).pipe(D.$as_tsv('date', 'size', 'name')).pipe(D.$show()).pipe(D.$on_finish(handler));
        for (i = 0, len = probes.length; i < len; i++) {
          probe = probes[i];
          D.send(input, probe);
        }
  ...
  this["(v4) $as_json_list (2a)"] = function(T, done) {
    ...
    f = function(path, handler) {
      ...
      })(this))).pipe(D.$on_stop((function(_this) {
        return function(send) {
          return send(']\n');
        };
  ...
  this["(v4) README demo (2)"] = function(T, done) {
    ...
    input = D.new_stream();
    input.pipe(D.$split()).pipe($show()).pipe($count()).pipe(D.$on_finish(done));
  ...
  this["(v4) README demo (3)"] = function(T, done) {
    ...
    input = D.new_stream();
    input.pipe(D.$split()).pipe($as_number()).pipe($add(12)).pipe(D.$sort()).pipe($show()).pipe(D.$on_finish(done));
    D.send(input, "20\n10\n50\n40\n30\n");
    D.end(input);
    return null;
  };
  ...
  this["(empty-string) can send empty strings (w/out pipeline)"] = function(T, done) {
    ...
    })(this))).pipe($validate_probes(T, matchers)).pipe(D.$on_finish(done));
    D.send(input, 'A text');
  ...
  this["(empty-string) duplexer2 works with empty strings"] = function(T, done) {
    var MSP, cause_error, collect_1, collect_2, collector_1, collector_2, filter, handler, input, matchers_1, matchers_2, pipeline, receiver, sender, show;
    MSP = require('mississippi');
    ...
    pipeline = (require('duplexer2'))({
      objectMode: true
    }, receiver, sender);
    input.pipe(pipeline).pipe(D.$on_finish((function(_this) {
      return function() {
  ...
  this["(empty-string) new D.duplex, new_stream from pipeline work with empty strings"] = function(T, done) {
    ...
    input.pipe(confluence).pipe(D.$on_finish((function(_this) {
      return function() {
        T.eq(collector_1, matchers_1);
        T.eq(collector_2, matchers_2);
        return done();
  ...
  this["(v4) $on_first, $on_last not called in empty stream (1)"] = function(T, done) {
    ...
    input.pipe(D.new_stream({
      pipeline: [$top(), $bottom()]
    })).pipe(D.$on_finish((function(_this) {
      return function() {
        T.eq(count, 0);
        return done();
      };
  ...
  this["(v4) $on_first, $on_last called in empty stream when tagged 'null' (1)"] = function(T, done) {
    ...
    count = 0;
    $top = function() {
      return D.$on_first('null', function(event, send) {
        count += +1;
    ...
    $bottom = function() {
      return D.$on_last('null', function(event, send) {
        count += +1;
  ...
  this["(v4) $on_first, $on_last, $on_start, $on_stop work as expected (1)"] = function(T, done) {
    ...
    has_ended = false;
    input = D.new_stream();
    input.pipe(D.$on_first((function(_this) {
      return function(data, send) {
        return null;
      };
    })(this))).pipe(D.$on_first((function(_this) {
      return function(data, send) {
        return send("{" + data + "}");
      };
  ...
  this["(v4) $on_first, $on_last, $on_start, $on_stop work as expected (2)"] = function(T, done) {
    ...
    has_ended = false;
    input = D.new_stream();
    input.pipe(D.$on_first('null', (function(_this) {
      return function(data, send) {
        return null;
      };
    })(this))).pipe(D.$on_first('null', (function(_this) {
      return function(data, send) {
        return send("{" + data + "}");
      };
    })(this))).pipe(D.$on_last('null', (function(_this) {
      return function(data, send) {
        return null;
      };
    })(this))).pipe(D.$on_last('null', (function(_this) {
  ...
  if (module.parent == null) {
    ...
    /* ----------------===#O#===-------------------- */
    include = ["(v4) _new_stream_from_path (2)", "(v4) _new_stream_from_pipeline (1a)", "(v4) _new_stream_from_pipeline (3)", "(v4) _new_stream_from_pipeline (4a)", "(v4) _new_stream_from_pipeline (4b)", "(v4) _new_stream_from_text", "(v4) _new_stream_from_text (2)", "(v4) observer transform called with data `null` on stream end", "(v4) D.new_stream", "(v4) stream / transform construction with through2 (1)", "(v4) D._new_stream_from_pipeline", "(v4) $async with method arity 3 (1)", "(v4) $lockstep 1", "(v4) $lockstep fails on streams of unequal lengths without fallback", "(v4) $lockstep succeeds on streams of unequal lengths with fallback", "(v4) $batch and $spread", "(v4) streams as transforms and v/v (1)", "(v4) streams as transforms and v/v (2)", "(v4) file stream events (1)", "(v4) transforms below output receive data events (1)", "(v4) transforms below output receive data events (2)", "(v4) _new_stream_from_url", "(v4) new_stream README example (1)", "(v4) new_stream README example (2)", "(v4) new_stream README example (3)", "(v4) _new_stream_from_path with encodings", "(v4) _new_stream_from_path (raw)", "(v4) new new_stream signature (1)", "(v4) new new_stream signature (2)", "(v4) _new_stream_from_path (1)", "(v4) $split_tsv (3)", "(v4) $split_tsv (4)", "(v4) read TSV file (1)", "(v4) TSV whitespace trimming", "(v4) $split_tsv (1)", "(v4) $intersperse (1)", "(v4) $intersperse (2)", "(v4) $intersperse (3)", "(v4) $intersperse (3a)", "(v4) $intersperse (4)", "(v4) $join (1)", "(v4) $join (2)", "(v4) $join (3)", "(v4) $as_json_list (1)", "(v4) $as_json_list (2)", "(v4) $as_json_list (2a)", "(v4) $as_json_list (2b)", "(v4) $as_json_list (2c)", "(v4) $as_json_list (3)", "(v4) symbols as data events (1)", "(v4) symbols as data events (2)", "(v4) $as_tsv", "(v4) $batch (1)", "(v4) $batch (2)", "(v4) all remit methods have opt-in end detection (1)", "(v4) all remit methods have opt-in end detection (2)", "(v4) all remit methods have opt-in end detection (3)", "(v4) all remit methods have opt-in end detection (4)", "(v4) README demo (1)", "(v4) README demo (2)", "(v4) README demo (3)", "(v4) $async only allows 3 arguments in transformation (1)", "(v4) $sort 1", "(v4) $sort 2", "(v4) $sort 3", "(v4) $sort 4", "(v4) $sort 5", "(v4) $sort 6", "(empty-string) can send empty strings ($split) (1)", "(empty-string) can send empty strings ($split) (2)", "(empty-string) can send empty strings (w/out pipeline)", "(empty-string) can send empty strings (w/ pipeline)", "(empty-string) duplexer2 works with empty strings", "(empty-string) new D.duplex, new_stream from pipeline work with empty strings", "$tabulate", "(v4) $on_first, $on_last not called in empty stream (1)", "(v4) $on_first, $on_last called in empty stream when tagged 'null' (1)", "(v4) $on_first, $on_last, $on_start, $on_stop work as expected (1)", "(v4) $on_first, $on_last, $on_start, $on_stop work as expected (2)"];
    this._prune();
    this._main();
  }
